...
WEBブラウザを立ち上げ、以下でアクセスします。  
http://localhost:5173/  

初回は、ユーザーの登録から開始しますが、確認用のメールは送信しません。    
代わりにバックエンドのログに確認用のURLが表示されますので、ブラウザでアクセスしてください。  
また、管理者権限も持っていますが、直接テーブルを更新する必要があります。  
...
```
※ /api を使えない場合は、ベースパスを変更できます。(/bbs_api/apiにする場合)　  
   フロントは、.env.production を作成し、VITE_API_BASE_PATH=/bbs_api  
   バックグエンドは、.env に API_BASE_PATH=/bbs_api  
   WEBサーバーのプロキシの設定も変更します。  
